...
function ScannersRef() {
  ...
  useEffect(() => {
    const beaconsRef = ref(db, '/localiza/dictionary/scanners');
  ...
  const writeToDatabase = () => {
    update(ref(db, `/localiza/dictionary/Scanners`), {
      [address]: todo,
    });
    setTodo("");
  };
  ...
  const handleSubmitChange = () => {
    remove(ref(db, `/localiza/dictionary/Scanners/${address}`))
      .then(() => {
        const updateData = {
          [address]: todo,
        };
        return update(ref(db, `/localiza/dictionary/Scanners`), updateData);
      })
      .then(() => {
        setAddress("");
        setTodo("");
  ...
  const handleDelete = (todo) => {
    remove(ref(db, `/localiza/dictionary/Scanners/${todo.mac}`));
  };
